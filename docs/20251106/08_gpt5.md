...
* If still busy: reply `{:error, %Error{type: :transport, message: "backpressure"}}`
...
*All user calls in “not ready” states return the **struct** form: `{:error, %Error{type: :state, message: "...", data: %{state: <state>}}}`.*
...
| :starting      | user_call(_method, ...)     |                                  | reply {:error, %Error{type: :state, data: %{state: :starting}}}| :starting    |
...
| :initializing  | user_call(...)              |                                  | reply {:error, %Error{type: :state, data: %{state: :initializing}}}| :initializing|
...
| :backoff       | user_call(...)              |                                  | reply {:error, %Error{type: :state, data: %{state: :backoff}}} | :backoff     |
