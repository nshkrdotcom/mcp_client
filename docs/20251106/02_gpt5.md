...
* Collapse error types under `McpClient.Error` with fields `:type, :message, :details, :server_error, :code`.
...
defmodule McpClient.Error do
  ...
  def message(%__MODULE__{message: m, type: t, code: c}), do: "[#{t}#{if c, do: "/#{c}", else: ""}] #{m}"
end
```
...
* Guard all decode errors and map to `%Error{type: :protocol}` with the offending payload size (but never log the entire body at `:info`).
...
def handle_info({:timeout, id}, state) do
  case :ets.take(:mcp_requests, id) do
    [{^id, from, _tref}] ->
      ...
      GenServer.reply(from, {:error, %McpClient.Error{type: :timeout, message: "request timed out"}})
    _ -> :ok
  end
  {:noreply, state}
end
